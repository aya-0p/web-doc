<!-- title 自己紹介 -->
<!-- update 2026-05-01 22:00 -->

# 自己紹介

## 目次

- [自己紹介](#自己紹介)
  - [目次](#目次)
  - [誰](#誰)
    - [一般人として](#一般人として)
    - [X(Twitter) の人として](#xtwitter-の人として)
    - [ぼ民として](#ぼ民として)
    - [開発者として](#開発者として)
    - [合成音声キャラクターとして](#合成音声キャラクターとして)
  - [趣味](#趣味)
    - [鉄道旅行](#鉄道旅行)
    - [その他の旅行](#その他の旅行)
    - [合成音声関係](#合成音声関係)
    - [料理](#料理)
  - [技術面](#技術面)
    - [Misskey サーバー](#misskey-サーバー)
    - [web サーバー](#web-サーバー)
    - [DiscordBot](#discordbot)
    - [misskey2x](#misskey2x)
    - [discord-song](#discord-song)
    - [tts](#tts)
    - [いろいろ Server (開発中)](#いろいろ-server-開発中)
  - [各種 SNS など](#各種-sns-など)
    - [X(Twitter)](#xtwitter)
    - [Misskey](#misskey)
    - [Bluesky](#bluesky)
    - [GitHub](#github)
    - [ニコニコ動画](#ニコニコ動画)
    - [Discord](#discord)
    - [現在未使用](#現在未使用)
  - [Other Accounts](#other-accounts)
    - [EMail](#email)
  - [Links](#links)
    - [WebSite](#website)
    - [Amazon ほしいかもしれないものリスト](#amazon-ほしいかもしれないものリスト)

## 誰

### 一般人として

- 名前: (非公開)
- 誕生日: (非公開) 年 3 月 29 日
- 所属: (非公開)大学

### X(Twitter) の人として

- 名前: [あや〜 (@ay_You_are_Hope)](https://x.com/ay_You_are_Hope)
- 昔は You are Hope というゲーム ( 旧 One Hour One Life for moblie ) でいろいろ遊んでいた人

### ぼ民として

- 名前: [あや (@aya\_@voskey.icalo.net)](https://voskey.icalo.net/@aya_)
- [追加情報はここ](#合成音声関係)

### 開発者として

- 名前: [aya-0p](https://github.com/aya-0p/)

### 合成音声キャラクターとして

[ここを参照](../voice/README.md)

## 趣味

### 鉄道旅行

代表的な旅行：北海道旅行 (2025)

<iframe src="https://voskey.icalo.net/embed/clips/a9b8km9tfu?maxHeight=700" data-misskey-embed-id="v1_01610775-48b6-4115-b5fd-4bb437dfed25" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="border: none; width: 100%; max-width: 500px; height: 300px; color-scheme: light dark;"></iframe>

他...

- 北海道旅行 (2025 年, 北海道全域など)
- ボイボ寮祭 (2024 年, 宮城県白石市)
- 中央線・飯田線 (2024 年, 岐阜県中津川市・長野県塩尻市・長野県下諏訪町)
- ボイボ寮祭 (2024 年, 神奈川県川崎市)
- 関東旅行 (2023 年, 東京都・群馬県前橋市・栃木県宇都宮市・千葉県千葉市など)

### その他の旅行

最新の旅行：韓国 (2026)

<iframe src="https://voskey.icalo.net/embed/clips/alqhsxa0p6?maxHeight=700" data-misskey-embed-id="v1_alqhw2qwqz" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="border: none; width: 100%; max-width: 500px; height: 300px; color-scheme: light dark;"></iframe>

他...

- ニュージーランド（2026 年, オークランド）
- 韓国 (2026 年, ソウル)
- [韓国 (2025 年, ソウル)](https://voskey.icalo.net/clips/a9bck9jf4v)
- 東京・仙台 (2024 年, 東京都・宮城県仙台市)
- 豊橋 (2024 年, 愛知県豊橋市)
- 掛川 (2023 年, 静岡県掛川市など)
- サークル旅行 (2023 年, 静岡県熱海市・沼津市)

### 合成音声関係

琴葉姉妹好きです
<iframe src="https://voskey.icalo.net/embed/notes/al7rblunlf" data-misskey-embed-id="v1_alqhzfdqtc" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="border: none; width: 100%; max-width: 500px; height: 300px; color-scheme: light dark;"></iframe>
<iframe src="https://voskey.icalo.net/embed/notes/9nk8geddzu" data-misskey-embed-id="v1_eeb85f5d-7302-42c1-8e07-1dca5b81b4c1" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="border: none; width: 100%; max-width: 500px; height: 300px; color-scheme: light dark;"></iframe>

### 料理

少し遡れば料理の写真が出てくると思います

<iframe src="https://voskey.icalo.net/embed/user-timeline/9kre3ml337?maxHeight=700" data-misskey-embed-id="v1_be035e14-7a37-40f5-85ca-05a1b69a234a" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" style="border: none; width: 100%; max-width: 500px; height: 300px; color-scheme: light dark;"></iframe>

## 技術面

### Misskey サーバー

<https://mi.ay0.org>

### web サーバー

ここ (<https://ay0.org>)

内容は<https://github.com/aya-0p/web-doc>で公開

言語：JavaScript, TypeScript (Node.js)

### DiscordBot

Discord で送信したものを読み上げる

言語：JavaScript, TypeScript, C++ (Node.js)

### misskey2x

Misskey で送信したメッセージを X(Twitter)に転送する

言語：TypeScript (Node.js)

### discord-song

Discord で送信したメッセージから VOICEVOX キャラクターが歌ってくれる

言語：TypeScript (Node.js)

<https://github.com/aya-0p/discord-song>

### tts

VOICEVOX のキャラクターに [voicevox_core](https://github.com/voicevox/voicevox_core) を用いずに喋らせる

言語：Rust

<https://github.com/aya-0p/tts>

### いろいろ Server (開発中)

以下の機能をまとめたサーバ

- Web Server
- Discord Bot
- Misskey Copy (Misskeyに投稿した内容を別のMisskeyやXに転送する)
- 部屋の温度監視
- メールアドレス作成

言語：Rust

## 各種 SNS など

- 複数あるものは先頭がメイン
- ここに表示されている以外のアカウントも存在します

### X(Twitter)

- [あや～(@ay_You_are_Hope)](https://twitter.com/ay_you_are_hope)

### Misskey

- ぼすきー
  - [あや/文山 彩(@aya\_@voskey.icalo.net)](https://voskey.icalo.net/@aya_)
- Misskey.io
  - [あや～(@aya\_@misskey.io)](https://misskey.io/@aya_)
- aya's misskey
  - [aya(@aya@mi.ay0.org)](https://mi.ay0.org/@aya)

### Bluesky

- aya-voskey
  - [あや/文山 彩(@aya-voskey.mi.ay0.org.ap.brid.gy)](https://bsky.app/profile/aya-voskey.mi.ay0.org.ap.brid.gy)
  - ぼすきーの内容をaya's misskeyにコピーした内容のブリッジです

### GitHub

- [Aya(aya-0p)](https://github.com/aya-0p)

### ニコニコ動画

- [あや](https://www.nicovideo.jp/user/46548269)

### Discord

- aya(aya_0p)
- あゆ(ayu_1)

### 現在未使用

- Misskey.io
  - [あや(@aya_0p@misskey.io)](https://misskey.io/@aya_0p)
- Mirrativ
  - [あや](https://mirrativ.com/user/104197174)
- YouTube
  - [あや](https://www.youtube.com/channel/UC5musEOBJXLA36RD7qV1xEA)

## Other Accounts

### EMail

- <main@ay0.org>
- <misskey@ay0.org> Misskey 関連はこちらへ

## Links

### WebSite

- <https://ay0.org>

### Amazon ほしいかもしれないものリスト

- <https://www.amazon.jp/hz/wishlist/ls/147RH8L0UWO0L>

<script defer src="https://voskey.icalo.net/embed.js"></script>
